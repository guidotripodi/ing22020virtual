...
public class StackAr {
	...
	public static boolean repOkFunction(int readIndex, Object[] elems){
		boolean result = false;
		if(elems != null && readIndex >= -1 && readIndex < elems.length){
			result = true;
			for (int i = readIndex + 1; i < elems.length; i++) {
				result = result && elems[i] == null;
			}
	...
	public boolean repOK() {
		return repOkFunction(this.readIndex, this.elems);
